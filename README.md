...
All values live under the `Config` JSON key in `appsettings.json`. See [Config.md](Config.md) for the full reference:
...
The app listens on plain HTTP (`http://127.0.0.1:5000`); nginx handles TLS termination and forwards `X-Forwarded-*` headers. See [Nginx.md](Nginx.md) for:
